        ...
        step_preprocessing_args = prep_processor.run(
            ...
            outputs=[
                ProcessingOutput(
                    ...
                    destination=os.path.join(target_path,'mr','train-data'),
                ),
                ProcessingOutput(
                    output_name="validation-data",
                    source=os.path.join(prefix_prep, "output", "validation"),
                    destination=os.path.join(target_path,'mr','validation-data'),
                ),
                ProcessingOutput(
                    output_name="test-data",
                    source=os.path.join(prefix_prep, "output", "test"),
                    destination=os.path.join(target_path,'mr','test-data'),
                )
            ],
            arguments=[
...
if __name__=="__main__":
    ...
    try:
        ...
        publish_sns(region_name=args.config.get_value('COMMON','region'),
                    ...
                    error_type="MR training pipeline 실행 중 에러",
                    error_message=e,
